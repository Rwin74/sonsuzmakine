...
for(const file of pages){const html=fs.readFileSync(file,'utf8');const route='/'+path.relative(root,path.dirname(file)).replaceAll('\\','/').replace(/^\.$/,'').replace(/\/$/,'');const title=html.match(/<title>(.*?)<\/title>/)?.[1];if(!title)errors.push(`${route}: başlık yok`);else if(titles.has(title))errors.push(`${route}: yinelenen başlık`);else titles.add(title);for(const re of [/name="description"/,/rel="canonical"/,/application\/ld\+json/,/<h1[ >]/])if(!re.test(html))errors.push(`${route}: ${re} yok`);if((html.match(/<h1[ >]/g)||[]).length!==1)errors.push(`${route}: H1 sayısı bir değil`);if(!html.includes('Gıda ve Toptancılar Sitesi')||!html.includes('7133. Sokak No: 5')||html.includes('Saraylar Mah.'))errors.push(`${route}: adres güncel değil`);for(const [,json] of html.matchAll(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/g)){try{JSON.parse(json)}catch{errors.push(`${route}: geçersiz yapılandırılmış veri`)}}for(const [,href] of html.matchAll(/href="(\/[^"?#]*)"/g)){if(href.startsWith('/assets/')||href==='/favicon.svg'||href==='/favicon.png')continue;const target=path.join(root,href,'index.html');if(!fs.existsSync(target))errors.push(`${route}: kırık bağlantı ${href}`)}for(const [,src] of html.matchAll(/<img[^>]+src="(\/[^"?#]*)"/g)){if(!fs.existsSync(path.join(root,src)))errors.push(`${route}: eksik görsel ${src}`)}}
